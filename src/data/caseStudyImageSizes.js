 ...
 * `thumbnailPosition` tunes 4:3 Resources grid crops only.
 */
...
/** Focal point for 4:3 thumbnail crops on the Resources case-study grid. */
export function getCaseStudyThumbnailObjectPosition(src) {
  const meta = getCaseStudyImageSize(src);
  if (!meta) return 'center top';
  if (meta.thumbnailPosition) return meta.thumbnailPosition;
  return meta.width > meta.height ? 'center' : 'center top';
}
